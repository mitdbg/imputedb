...
\item \resp{We extended the original paper two additional popular imputation techniques: mean value imputation and hot-deck imputation.}

\item \resp{We extended experiments to include the new operators and provide more details on results for $\alpha$. We also added the Pareto frontiers for various plans and
explored planning times for more complex queries.}

\item \resp{We added a section discussing how to construct the imputation-related functions used by our cost model, which represent the API for a new imputation operator.}
We provide details on the definition of these functions for all imputation methods used.

\item \resp{We included a new section that provides a roadmap for implementing \ProjectName{} on top of an existing database.}
\end{itemize}
  ...
  \item W3. Reducing runtime to 10s is excellent, but comes at a very serious accuracy cost (3-5x worse SMAPE). Worse still, there's no way for the user to differentiate between the cases of Q4 and Q8 at runtime. 24\% error relative to direct imputation is huge.
    ...
    \resp{
      ...
      been dropped (after execution), so that situations such as those observed in Q8 can be identified by the user. We've added this detail to the paper.
    }
 ...
 \item S1: "Furthermore, the results obtained with this approach are similar in quality to those obtained with the traditional imputation approach between 0 and 24 percent" 24 percent error is pretty bad.
  ...
  \item S4: All of the workload queries are (data-size-independent) AVG queries. What about other types of aggregates (e.g., SUM, COUNT)?
    ...
    \resp{Though we do not show this explicitly, we repeat all of the queries using COUNT
      ...
      and the AVG value estimated. We have noted this in the experiments section now.}
...
%      times further.
  ...
  \item D5. Can you have plans with a mix of drop and impute operators?
    ...
    \resp{Yes, we have now highlighted this for clarity in the paper as well.\todo{add specific section}}
  ...
  \item D6. During plan enumeration, can you safely prune plans at some point, similar to what a traditional optimizer would do?
    ...
    now clarified in the paper). We found that the size of these Pareto sets is small enough that we did not need to prune further.
  ...
  as gradient descent. We've clarified this technique in the section discussing this imputation operation.
}
  ...
  \item D9. Additional experiments I suggest to include:
    ...
    \resp{
      ...
      which can result in many duplicate plans (since different $\alpha$ values may result in the same plan), we extended the results section to include a subsection that presents the final frontiers for
