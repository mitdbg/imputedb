...
significant speedup in query execution while mitigating the number of dropped rows. Unlike previous work, the focus of our work is not on the imputation algorithms themselves (we can employ almost any such algorithm), but rather on placing imputation operations
...
percent --- in our empirical results over real-world data sets (see~\Cref{sec:experiments}
for details).
