...
several plans on an optimal frontier. \Cref{fig:quality-plan} shows a quality-optimized plan that uses the impute operator $\mu$,
...
On the other hand, \Cref{fig:fast-plan} shows a performance-optimized plan that uses the drop operation $\delta$ instead of filling in missing values.
