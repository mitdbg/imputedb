...
\caption{The operators $\sigma$, $\pi$, $\bowtie$, and $g$ are the standard relational selection, projection, join, and group-by/aggregate, $\mu$ and $\delta$ are specialized imputation operators (\Cref{sec:operators}). The imputation operators are highlighted.}
\end{figure}
...
\Cref{fig:quality-plan} shows a quality-optimized plan that uses the impute operation $\mu$,
...
\review{
As an alternative strategy, the analyst can start by executing their query with $\alpha=1$ and incrementally reducing $\alpha$, as long as execution of the query completes within some user-defined time bound.
\ProjectName{}'s planning algorithm can expose the final set of plans with optimal time-quality trade-offs, which allows the user to adjust their choice of $\alpha$ to select from these plans.
}
