...
\subsection{Queries}
We collect a set of queries (\Cref{fig:queries}) that we think are both representative,
...
Our first set of queries is on the CDC data (\Cref{fig:queries-cdc}). In~\Cref{q1}, we calculate the average height for
...
Our second set of queries is on the FCC data (\Cref{fig:queries-fcc}). In~\Cref{q5}, we calculate the average income for
...
\Cref{fig:runtimes} provides a summary of the performance results. The first plot
...
aggregate query over the ACS dataset in our final experiment(\Cref{fig:query-acs}).
