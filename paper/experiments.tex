...
\begin{figure}
\todo{Relabel 9 somehow, since it is no longer in table of queries but in lined later}
...
\caption{Symmetric-Mean-Absolute-Percentage-Error for queries run under different $\alpha$
    ...
    lower error than queries optimized for efficiency ($\alpha=1$). Count values reflect the SMAPE over the number of tuples
    used for each configuration with respect to the number of tuples used when executing the query after base table imputation. A higher
    count difference reflects more potential for errors.}
\label{table:smape}
\end{table}
