...
For each query, we evaluate \todo{update: three different configurations}: \ProjectName{} optimizing for
...
queries are an \todo{update/validate: order-of-magnitude faster than imputing on the base tables. We get another order-of-magnitude speedup when optimizing for performance}. This performance
...
to the base-imputation approach is low---\todo{update: between \lowsmapealphazero{} and
...
\caption{\ProjectName{}'s final Pareto frontier for \Cref{q1} and \Cref{q6}. 
...
\caption{Planning times are sub-second for up to 5 joins. For more joins, we implement an approximation to Pareto sets.
This reduces planning time linearly with respect to the reduction in the final frontier size. Users can increase the level of
approximation for more aggressive planning reductions.
}
\label{fig:planning-times}
\end{figure}

\todo{Update: Planning times for our benchmark queries are reasonable, with a median planning time of \planningmediantime{} ms. over the benchmark queries, constituting \planningruntimepercent{}\% of total runtime.
In all cases, the optimizer returned a query plan within \planningmaxtime{} ms.}
The choice of $\alpha$ has no impact on planning times, because query selection is done after collecting the Pareto frontier.
...
\Cref{fig:planning-times} shows the average planning time on a log scale for queries of the CDC data with an increasing number of joins. As the number of tables requiring imputation grows,
so does the search space and corresponding planning time. Planning time for queries involving 1--5 joins are sub-second. While a useful subset of exploratory queries can be expressed
with a smaller number of joins, we extended \ProjectName{} to support an approximation of Pareto sets to improve planning times for queries with 6 or more joins. Approximate Pareto sets
displayed a reduction in planning time linear with respect to the reduction in size of the final frontier. Users can increase the level of approximation to reduce planning times further.
