...
To evaluate the performance of our approach, we implement a prototype system in Java, following a traditional iterator model.
...
\Cref{q9} joins the freeCodeCamp data with data provided by the World Bank which summarizes GDP per-capita across various countries~\cite{worldbank-data}.
...
imputation on the base tables. In a real use-case, the data do not have ground-truth
anyways, so the appropriate comparison is to this full-fledged impute on the base table.
...
The Count columns in \Cref{table:smape} show the percentage difference between the number of tuples in the aggregate for $\alpha = 0$ and $\alpha = 1$ and running the query on the imputed base table.
