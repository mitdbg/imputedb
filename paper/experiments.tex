...
\Cref{table:smape} shows the Symmetric-Mean-Absolute-Percentage-Error (SMAPE)~\cite{Makridakis2000451} for \ProjectName{}'s query results compared to the baseline, using the reference chained-equation decision trees model.
...
\nullv{}s. Thus, it is important to recognize the tradeoffs between quality and
performance for the specific use case.

\review{
We also experiment with mean-value imputation and hot
deck imputation, as outlined in \Cref{sec:imputation}. One approach to
...
execution. \todo{be quantitative: In terms of accuracy, relative errors using hot deck are close to
...
Our benchmark queries performed well for both cases of \texttt{AVG} and \texttt{COUNT} aggregates, so we expect \ProjectName{} would provide similar performance for \texttt{SUM}, with 
...
In contrast, \ProjectName{} executes the query \lstinline{SELECT AVG(c0) FROM acs} over the ACS dataset (also using chained-equation decision trees imputation) in \acsimputedbzeroresult{} when optimizing for quality and \acsimputedboneresult{} when optimizing for runtime. Given the runtime difference, we can run approximately 5,000 queries before taking more time than full imputation.
...
Planning times for our benchmark queries are reasonable, with mean planning
times across the benchmark queries ranging from \planninglowtime{} ms to
\planninghightime{} ms, and ranging from \planninglowtimepct{} to
\planninghightimepct{} as a share of total runtime.  In all cases, the
optimizer returned a query plan within \planningmaxtime{} ms. The choice of
$\alpha$ has no impact on planning times, because query selection is done after
collecting the Pareto frontier.
