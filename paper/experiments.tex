...
\caption{Runtimes for the queries in~\Cref{tbl:queries}. $\alpha=0$ (resp. $\alpha=1$) is \ProjectName{} run at the highest (resp. lowest) quality setting. Baseline is full table imputation followed by running the query.}
...
In our case, the relevant comparison is against traditional imputation on the base table, given the unavailability of ground truth.
...
For example, if we run CART imputation on the full ACS dataset, it completes in \acsbaseresultminutes{}.

In contrast, \ProjectName{} executes the following query:
\begin{lstlisting}[breaklines]
SELECT AVG(c0) FROM acs;
\end{lstlisting}
over the ACS dataset (also using CART imputation) in \acsimputedbzeroresult{} when optimizing for quality and \acsimputedboneresult{} when optimizing for runtime.
...
\caption{Planning times for varying numbers of joins. \todo{update with new table and values}}
  % Planning times are sub-second for up to 4 joins. For 5+  joins planning times rise. Many practical queries
...
\todo{Update: Planning times are reasonable, with a median planning time of \planningmediantime{} ms. over the benchmark queries, constituting \planningruntimepercent{}\% of total runtime.
...
We explored the limits our planning algorithm by constructing a series of queries with increasing number of joins over the CDC tables.
...
\todo{update: For queries with 7 joins, planning time increases substantially to 29 seconds, on average.
  It is possible to reduce planning time by approximating the Pareto set, particularly for queries with a large number of joins.
  With this optimization, the queries with 7 joins have an average planning time of 16 seconds (2 seconds SE).}
