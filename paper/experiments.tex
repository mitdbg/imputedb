...
We use survey data collected by the 
Center for Disease Control and Prevention (CDC) in the United States. We
...
results, amongst others. Our dataset is available for direct download on the CDC's organization site on 
kaggle.com, a popular data science website.

There are a total of 6 tables associated with the NHANES data set. We use three tables:
...
exploration of queries, we focused on a subset of the attributes for each table.~\Cref{table:nhanes-description}
...
\begin{table}
    ...
    \end{subtable}
    \caption{Missing value distribution within each table/attribute in CDC NHANES 2013-2014 data}
...
pre-processed, but where many missing values remain. It is
...
columns. We chose 17 attributes, which are outlined in~\Cref{table:fcc-description}, along with the percentage of missing values.
...
\subsection{Queries}
We collect a set of queries (\Cref{fig:queries}) that we think are both representative and interesting
to plan. They could reasonably be written by a user in the course of data analysis.
...
Queries 5-8 are on the fCC data. In~\Cref{q5}, we calculate the average income for
survey participants, based on their bootcamp attendance. In~\Cref{q6} estimates the
average age of women from the United States who participated. In~\Cref{q7}, we
calculate the average amount of money survey participants with student debt destined to
learning based on their school degree. In~\Cref{q8}, we join the fCC data with a
...
\caption{Comparing \ProjectName{} runtimes for each query
    ...
    imputations are used (\Cref{subfig:base-runtime-queries})}
\label{fig:runtimes}
\end{figure}
...
\Cref{table:smape} shows the Symmetric-Mean-Absolute-Percentage-Error (SMAPE) for
\ProjectName{}'s query results when compared to running imputation on the base tables and
executing the query on the cleaned data. Each query ran for 200 iterations in both settings
...
approach are low in all cases\todo{give actual value}, indicating that on-the-fly imputation produces similar
...
\caption{Symmetric-Mean-Absolute-Percentage-Error for queries run under different $\alpha$
    ...
    $100.0$, or $NaN$ indicate uninformative values.} %TODO MJS
\label{table:smape}
\end{table}
...
aggregate query over the ACS dataset in our final experiment(\Cref{query-acs}).
