...
\todo{update with new table and values}
}
...
We explored the limits our planning algorithm by constructing a series of queries with increasing number of joins.
We used the CDC tables and evaluated a combination of self-joins and joins across reference tables.
Planning times for queries involving 1--5 joins, which is a practical value for real world exploratory queries, take less than a second.
\todo{update: For queries with 7 joins, planning time increases substantially to 29 seconds, on average. We introduced an optimization that reduces
