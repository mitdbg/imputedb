...
\subsection{Queries}
\todo{Add more queries: we want ~10 (balanced between the two datasets), explain them below}
...
Table~\ref{table:rmse} shows the Root-Mean-Squared-Error (RMSE) for \ProjectName{}'s query results when compared to running imputation on the base tables and executing the query on the
cleaned data. Each query was run 10 iterations in both settings and then results from the two approaches were paired up and compared tuple-wise. We average the
squared error across a single query before applying square root. We can see that optimizing for quality indeed reduces the RMSE of query results. In general, the RMSE relative to the
base-imputation approach are acceptable.

\begin{table}
\input{perf_summary.tex}
\caption{Root-Mean-Squared-Error for queries run under different $\alpha$ parameterizations relative to results when imputing on base table}
\label{table:rmse}
\end{table}
