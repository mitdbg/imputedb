...
The first set of queries is on the CDC data.
...
The second set of queries is on the freeCodeCamp data.
...
\Cref{fig:runtimes} shows the performance results. The quality-optimized
queries are an order-of-magnitude faster than imputing on the base tables. We get
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{figures/pareto_frontiers_plot.png}
...
Planning times are reasonable, with a median planning time of \planningmediantime{} ms. over the benchmark queries, constituting \planningruntimepercent{}\% of total runtime.
...
For queries with 7 joins, planning time increases substantially to 29 seconds, on average.
We introduced an optimization that reduces the plan space by maintaining an approximation of the Pareto set for queries involving a large number of joins.
