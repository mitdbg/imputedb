...
Census Bureau. We use a cleaned version of the 2012 Public Use Microdata Sample (PUMS). Given that the data had
...
so we expect \ProjectName{} would provide similar performance for \texttt{SUM}, with 
...
\review{We evaluate \ProjectName{} optimizing for quality ($\alpha=0$), optimizing for performance
...
\Cref{fig:runtimes} summarizes the performance results. The quality-optimized
...
deck imputation (\Cref{sec:imputation}). One approach to
...
imputations are identical regardless of where the operator is placed in the query
plan. Moreover, runtimes are essentially identical regardless of the value of
$\alpha$ because the time cost of the imputation is relatively low independent of
...
\review{Given the runtime difference, we can run approximately 5,000 queries before taking more time than full imputation.
