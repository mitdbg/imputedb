%\begin{algorithm}
  \begin{algorithmic}
    ...
    \If{$D_{may} \neq \emptyset$}
    \State $Q \gets Q \cup \{\mu_{D_{may}}(q), \delta_{D_{may}}(q) \}$
    \EndIf
    ...
    \Ensure{A set of plans for grouping and aggregating with $G$ and $A$.}
    ...
    \Require{$q$ is a query plan, $P$ is a set of attributes to be projected at the top of the plan.}
    ...
    \Ensure{An updated plan cache $Q'$ which contains at least one plan that combines all the tables in $T$.}
