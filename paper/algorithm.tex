...
\cameraready{
  ...
  \begin{equation*}
    \begin{aligned}
      T(q) = k &\times |C| \times \textsc{Card}(q) \\
      &\times \left(|\textsc{Clean}(q)| + |\textsc{Dirty}(q)| - 1\right) \times \log\ \textsc{Card}(q).
    \end{aligned}
\end{equation*}}
...
\begin{figure}
  ...
  \end{align*}
  \caption{\cameraready{Semantics of a plan cache $Q$. We use $\llbracket\ \rrbracket$ to mean evaluation. $T$ is a set of tables and $p$ is a query plan. The cache guaranteees that plans for the same set of tables either have distinct dirty sets or are Pareto optimal.}}
