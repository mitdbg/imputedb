...
\label{sec:cardinal}
The computation of $\textsc{Time}(q)$ and $\textsc{Penalty}(q)$ rely accurate cardinality estimates for the plan $q$ and its sub-plans.
...
\label{sec:quality}
We estimate the quality of the imputations in a plan $q$ using a heuristic measure $\text{Penalty}(q, P)$, which is computed as follows.

\begin{align*}
  \textsc{L}(q, f) &= \begin{cases}
  ...
  %   0 & q = \text{some table } t\\
  % \end{cases}\\
  \textsc{Penalty}(q, P) &= \frac{\textsc{L}(q, P)}{\textsc{L}(q, 1)}
\end{align*}

$\textsc{Penalty}(q, P)$ assigns a cost in $(0,1]$ to each imputation operator in the plan $q$ according to its estimated quality, then averages over the individual costs.
For example, each occurrence of $\delta$ incurs a cost of 1 (i.e.\ we lose all information that dropped tuples might have encoded).
Imputing \nullv{} fields with $\mu$ incurs a cost in $(0, 1]$ according to a cost function $P$, which is specific to the imputation method in use.

The choice of $P$ depends on the properties of the imputation method being used.
A well chosen $P$ should provide a reasonable estimate of the quality of imputation from the histogram describing the input data.
From this histogram, $P$ can obtain the number of attributes in its input, its cardinality, and an estimate of its variance.
Intuitively, imputation accuracy \textit{increases} when more complete attributes and complete tuples are available; correspondingly, the $P$ should \textit{decrease} as the number of values increases.
$P$ should never decrease to zero, as this would mean that it can perform perfect imputation.
It should reflect the intuition that the number of training observations has diminishing returns by decreasing more slowly as the number of values increases.
