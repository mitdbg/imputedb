...
with. We encode the fact that the number of training observations has diminishing returns for imputation quality,
and find that scaling the cell count by applying square root works well in practice.
...
and simply queries it for a time estimate. In our reference implementation,
...
attributes $C$ in a relation $R$, returning a new set of tuples $R'$.
...
In principle, new imputation algorithms can be added, possibly targeted to a specific
domain. However, \ProjectName{}'s planner will consider an arbitrary subset of the
attributes in any imputation, thus it is feasible to customize the \textit{type} of
algorithm but not necessarily a specific modelling relationships between attributes.
...
regression}~\cite{gelman2006data}) impute multiple missing attributes being imputed by iteratively fitting
