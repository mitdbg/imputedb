...
values. \cameraready{Attributes that contain missing values but that are not being imputed are
...
\cameraready{
  We define the time function as follows.
  Let $C$ be the set of attributes to be imputed and let $k$ be the number of cycles of the algorithm, where in each cycle there are
  $|\textsc{Dirty}(q)|$ iterations of fitting models for each dirty attribute.
  Then, we have
  \begin{equation*}
    \begin{aligned}
      T(q) = k \times &|C| \times \textsc{Card}(q) \times \\
      &\left(|\textsc{Clean}(q)| + |\textsc{Dirty}(q)| - 1\right) \times \log\ \textsc{Card}(q).
    \end{aligned}
\end{equation*}}
...
\[
    P(q) = \frac{|\textsc{Dirty}(q)|}{\sqrt{ \textsc{Attr}(q) \times \textsc{Card}(q)}}
\]
...
\begin{figure}
  ...
  \end{align*}
  \caption{\cameraready{Semantics of the plan cache access and insertion syntax. We use $\llbracket\ \rrbracket$ to mean evaluation. $Q$ is a plan cache, $T$ is a set of tables, and $p$ is a query plan. The cache guaranteees that plans for the same set of tables either have distinct dirty sets or are Pareto optimal.}}
