...
\todobox{this jumps around too much}{
Our optimizer searches (\Cref{sec:planning}) a restricted space (\Cref{sec:search-space}) of query plans containing imputation operators (\Cref{sec:operators}) for an optimal plan given an estimate of the runtime of the query, information lost through imputation operations, and a parameter trading off the two.

This plan is subject to two constraints (\Cref{sec:placement}).
...
Our planning algorithm is agnostic to the type of imputation used (\Cref{sec:imputation}), as the properties of the imputation are summarized by the cost model (\Cref{sec:cost-model}).}
...
can only be made once the set is final. Details are shown in~\Cref{fig:semantics-plan-cache}.
...
\todo{Stengthen complexity argument, as per Sam's feedback}
