...
\begin{definition}
We say that a plan $p$ with cost $\langle l, t \rangle$ dominates a plan $p'$ with cost $\langle l', t' \rangle$ if $l < l'$ and $t < t'$. We denote this as $\doms{p}{p'}$. 
\end{definition}
...
In order to pick a final plan from the frontier, our model introduces a parameter $\alpha$ which encodes an upper bound on the amount of information loss that a user is willing to tolerate relative to the minimum information loss possible given the imputation operations.\todo{Get rid of discussion of information loss. It's a technical term and we don't really tackle it.}
...
\todo{Similar algorithm for computing Time?}
...
plans encompassing all necessary tables is produced (Line~\ref{lst:line:join})\todo{Citation or explanation for join optimization algorithm}. This set is now extended for any necessary grouping and aggregation
...
then we could explore $\mathcal{O}(3^{\# of operators}) * plans$\todo{this is unclear. what is plans?}. 
