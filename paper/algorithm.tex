...
This set $Q$ will be the Pareto frontier of $S$~\cite{pareto1964cours}, and it contains the best option for all feasible trade-offs of $\textsc{Time}$ and $\textsc{Penalty}$ for the current query.
...
From these, $P$ can obtain the number of clean and dirty attributes in its input, their cardinalities, and approximations to distribution statistics such as mean and variance.
...
that the \textit{Impute} routine is CPU-bound, then we adjust
\textsc{Time} by some constant factor $f$, thereby scaling the result to be
comparable with IO-bound routines.

The form of $T$ depends on the specific imputation method, but there are various
principles useful in capturing the intuition of a practical choice.
$T$ should \emph{increase}
...
penalty ($P$) and time ($T$) functions (\Cref{sec:cost-model}) pertinent to the algorithm.
...
At the other end of the spectrum of complexity, an analyst could use a simple
method that replaces a missing value with the mean over available values
in that attribute, a common choice in exploratory data analysis known
as mean value imputation. 

We can take advantage of the histograms maintained by \ProjectName{}
to estimate the mean and construct a non-blocking impute operator.
...
distribution of the complete values. The heuristic $P$ remains the same, 
but we modify $T$ to account for the fact that hot-deck is blocking in our
implementation.
