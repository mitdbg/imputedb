...
the dropped tuples might have encoded). Imputing \nullv{} fields incurs a loss penalty $p \in (0, 1]$,
...
Pareto frontier corresponds to the collection of plans associate with the empty dirty set.
...
(\Cref{sec:cost}) which describe the new algorithm. In principle, new imputation algorithms
