...
The computation of $\textsc{Time}(q)$ and $\textsc{Loss}(q)$ rely on having an accurate cardinality estimate of a query plan $q$ and its sub-plans.
...
To add an alternate imputation strategy to \ProjectName{}, the analyst must provide a corresponding loss with a reasonable parameterization.
...
allows us to obtain the final set of plans that best trade computation
...
The planner (\Cref{algo:top-level-planner}, with helper functions in \Cref{algo:plan-helpers}) operates as follows.
...
\ProjectName{} is designed so that any imputation strategy, typically a predictive machine
learning model,
can be plugged in with minimal effort and without changes to the optimizer, simply by adding
time and loss functions (\Cref{sec:cost-model}) which describe the new algorithm.
An imputation strategy is a function $I(T, C) \rightarrow T'$ which fills in the missing
attributes $C$ in a set of tuples $T$, returning a new set of tuples $T'$.
In principle, new imputation algorithms
can be added, possibly targeted to a specific domain. The flexibility of this approach
aligns with the spirit of query optimization, which aims to reconcile performance with
...
regression trees to a subset of the data and the target imputation attribute. In each
iteration, the missing values of a single attribute are replaced with newly imputed values.
...
With each {\it epoch}, the quality of the imputation improves as values progressively
...
further work, multiple imputation could be another avenue of assessing
