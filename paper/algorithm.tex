...
\begin{align*}
  \textsc{Losses}(q) &= \begin{cases}
    ...
    \{\frac{1}{\sqrt{ \textsc{Attr}(q') * \textsc{Cardinality}(q') }} \} \cup \textsc{Losses}(q')  & q = \mu_C(q') \\
    \textsc{Losses}(q_1') \cup \textsc{Losses}(q_2') & q = q_1' \bowtie_\psi q_2' \\
...
note that if we remove our restriction on imputation operators (i.e. minimal vs maximal), and allow any arbitrary subset of attributes to be imputed,
