...
\cameraready{
Recall that $C$ is the set of dirty attributes that are being imputed in a
given operation, where $C \subseteq \textsc{Dirty}(q)$.
...
\begin{equation*}
    \begin{aligned}
    T(q) = k * &|C| * \textsc{Card}(q) * \\
               &(|\textsc{Clean}(q)| + |C| - 1) * log(\textsc{Card}(q)).
    \end{aligned}
\end{equation*}
...
\[
    P(q) = \frac{|C|}{\sqrt{ \textsc{Attr}(q) * \textsc{Card}(q)}}
\]
...
\begin{equation*}
\begin{aligned}
    P(q) &= \sum_{c \in C} \frac{1+\widehat{Var}(c)}{\textsc{Card}(q)} \\
    T(q) &\approx 0.
\end{aligned}
\end{equation*}
