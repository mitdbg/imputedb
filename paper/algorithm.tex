...
$\textsc{Penalty}(q, P)$ assigns a penalty in $(0,1]$ to each imputation operator in the
plan $q$ according to its estimated quality, then averages over the individual penalties.
For example, each occurrence of $\delta$ incurs a penalty of 1 (i.e.\ we lose all information that dropped tuples might have encoded).
Imputing \nullv{} fields with $\mu$ incurs a penalty in $(0, 1]$ according to a cost function $P$, which is specific to the imputation method in use.
