...
We say a plan $q^\alpha$ is $\alpha$-bound optimal if \[\textsc{Time}(q^\alpha) = \min_{q' \in Q} \textsc{Time}(q').\]
\end{definition}
}
...
\begin{figure}
  ...
  \end{align*}
  \caption{\cameraready{Semantics of a plan cache $Q$. $T$ is a set of tables and $p$ is a query plan. The cache guaranteees that plans for the same set of tables either have distinct dirty sets or are Pareto optimal.}}
