...
\begin{figure}
  ...
  \begin{align*}
    ...
    \text{ in }
    P' &= \left\{q ~\Bigg|~ \begin{aligned}
              q \in S,\\
              \textsc{Dirty}(q) = \textsc{Dirty}(p) \implies\\
              \not \exists q' \in S.\doms{q'}{q}
              \end{aligned}
            \right\}
  \end{align*}
  \caption{Semantics of a plan cache $Q$, where $T$ is a set of tables and $p$ is a query plan. \cameraready{When extending with a new plan $p$, the cache guarantees
  no dominated plans with the same dirty set are kept.}}
