...
For each sub-query, using the histogram information, we keep track of the estimated number of
null values in a column  ($\text{MissingCount}(c, Q)$), given the histograms associated with
...
Evaluation of the time complexity for \textit{Impute} is more thorny, as the properties of
...
practice, we find that a simple parameterization can be acceptable (yield intelligent query
...
associated time and loss functions --- could be insert without any change in design.
