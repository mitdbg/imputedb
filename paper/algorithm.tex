...
\begin{algorithm}
  \begin{algorithmic}
    \Require{$T$ is a table. $D$ is a set of attributes of $T$ that need to be imputed, and
      $C$ is a set of attributes of $T$ that has complete data }
    \Ensure{Returns an imputed $T$}
    
    \Function{ImputeWithCE-CART}{$H, op$}
...
\todo{Micah: Explanation edited a bit. In the algorithm, when we impute attribute $d$, we
condition on attributes $C \cup D \backslash \{d\}$. Hopefully the way I note it is clear;
feel free to modify otherwise.}
