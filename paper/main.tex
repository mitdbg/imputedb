...
approach. 
We calculate the number of tuples used to produce each aggregate
in our experiments, and find that when optimizing for quality between 0 to 10\% fewer tuples satisfy query constraints relative to the full-table imputation. When all tuples with missing values are dropped, the aggregate
is computed with 3 to 88\% fewer tuples.
\end{abstract}
